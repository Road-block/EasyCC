...
local function makeGreenBar(self)
	local text
	if type(self.spellID) == "number" then
		text = GetSpellInfo(self.spellID)
	else
		text = self.spellID
	end
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
...
local function CustomAddedCompileSpells(spell, prio, tab, customspellstring) --Adding a Custom Spell
	...
	if customspellstring then
		_G.EasyCCDB.customString[spell] = customspellstring
	end
	L.Spells:WipeSpellList(tab)
	tblinsert(_G.EasyCCDB.customSpellIds, { spell, prio, customspellstring, nil, nil, "Custom Spell", tab })
	tblinsert(L.spells[tab][1], 1, { spell, prio, customspellstring, nil, nil, "Custom Spell", tab })
		...
		for k, v in ipairs(L.spells[tab][l]) do
			...
			if spell == spellID then
				if prio == "Delete" then
					...
					L.spellIds[spell] = prio
					local customspellstring = _G.EasyCCDB.customString[spell]
					if customspellstring then
						customname = "Custom Spell"
					else
						customname = "Custom Priority"
					end
					tblinsert(_G.EasyCCDB.customSpellIds, { spell, prio, customspellstring, nil, nil, customname, tab }) --v[7]: Category Tab to enter spell
					local priotext = L[prio] or prio
					print(
						"|cff00ccffEasyCC|r : " .. "|cff009900Changed |r" .. spell .. " |cff009900to : |r" .. priotext
					)
				end
				break
			end
...
local function createDropdownAdd(opts,dropdown_val)
	local dropdown_name = "$parent_" .. opts["name"] .. "_dropdown"
	local menu_items = opts["items"] or {}
	local title_text = opts["title"] or ""
	local dropdown_width = 0
...
local function SetTabs(frame, numTabs, ...)
	...
	for i = 1, numTabs do
		...
		if tabs[i] == "Discovered Spells" then
			...
			tab.content.input.Ltext:SetTextColor(1, 0.75, 0, 1)
			...
			tab.content.inputText =
				CreateFrame("EditBox", tab:GetName() .. "CustomSpellsString", tab.content, "InputBoxTemplate")
			...
			tab.content.inputText:SetScript("OnChar", function(self, customspelltextstring)
				tab.content.inputText.customspelltextstring = self:GetText()
			end)
			...
			tab.content.inputText.Ltext:SetTextColor(1, 0.75, 0, 1)
			...
			local dropdown = createDropdownAdd(drop_opts,drop_val)
			dropdown:SetPoint("TOPLEFT", tab.content.inputText, "BOTTOMLEFT", 10, 3)
			dropdown:SetScale(0.85)

			tab.content.add =
				CreateFrame("Button", tab:GetName() .. "CustomSpellsButton", tab.content.input, "UIPanelButtonTemplate")
			...
			tab.content.add:SetScript("OnClick", function(self, addenemy)
				...
				local customspellstring = tab.content.inputText.customspelltextstring
				if spell then
					spell = tonumber(tab.content.input.customspelltext)
				else
					spell = tab.content.input.customspelltext
				end
				if drop_val and spell then
					CustomAddedCompileSpells(spell, drop_val, i, customspellstring)
				else
		...
		tab.content.reset =
			CreateFrame("Button", tab:GetName() .. "CustomSpellsButton", tab.content, "UIPanelButtonTemplate")
		tab.content.reset:SetSize(70, 22)
		tab.content.reset:SetScale(0.7)
		if tabs[i] == "Discovered Spells" then
		...
		tab.content.disable:SetScale(0.7)
...
function Spells:UpdateSpellList(i, typeUpdate)
	...
	for l = 1, #L.spells[i] do
		for x = 1, #L.spells[i][l] do
			local spellID, prio, customspellstring, instanceType, zone, customname, tab = unpack(L.spells[i][l][x])
			if spellID then
				local spellCheck = GetSpellFrame(spellID, c)
				if spellCheck then
					...
					if typeUpdate then
						...
						if type(spellID) == "number" then
							if instanceType == "arena" or instanceType == "pvp" then
								...
								if name then
									aString = substring(name, 0, 17) .. ": " .. substring(prio, 0, 6)
								else
									aString = "SPELL REMOVED: " .. spellID
								end
								local aString2 = " (" .. instanceType .. ")"
								local cutString1 = substring(aString, 0, 23)
							...
							elseif zone then
								...
								if customname and customspellstring then
									spellCheck.Text:SetText(
										cutString .. "\n" .. "(" .. customname .. ")" .. "\n" .. customspellstring
									)
					...
					spellCheck =
						CreateFrame("CheckButton", c:GetName() .. "spellCheck" .. spellID, c, "UICheckButtonTemplate")
					spellCheck.Green = CreateFrame(
						"CheckButton",
						spellCheck:GetName() .. "spellCheckGreen" .. spellID,
						spellCheck,
					...
					spellCheck:Show()
					spellCheck.Green:SetScale(0.4)
					spellCheck.Green:Show()

					spellCheck.icon =
						CreateFrame("Button", spellCheck:GetName() .. "Icon", spellCheck, "ActionButtonTemplate")
						...
						["changeFunc"] = function(dropdown_frame, dropdown_val)
							...
							if dropdown_val ~= prio then
								...
								if type(spell) == "number" then
									aString = substring(GetSpellInfo(spellID), 0, 17) .. ": " .. substring(prio, 0, 6)
										or "SPELL REMOVED: " .. spellID
									local cutString = substring(aString, 0, 23)
									if customspellstring then
										spellCheck.Text:SetText(
											cutString .. "\n" .. "(" .. customname .. ")" .. "\n" .. customspellstring
					...
					dropdown:SetScale(0.55)
					spellCheck.spellID = spellID
					spellCheck.Green.spellID = spellID
					if _G.EasyCCDB.spellDisabled[spellCheck.spellID] then
						spellCheck:SetChecked(false)
					else
						spellCheck:SetChecked(true)
					end --Error on 1st ADDON_LOADED
					spellCheck:SetScript("OnClick", function()
						GameTooltip:Hide()
						if not spellCheck:GetChecked() then
							_G.EasyCCDB.spellDisabled[spellCheck.spellID] = true
						else
							_G.EasyCCDB.spellDisabled[spellCheck.spellID] = nil
						end
					...
					if _G.EasyCCDB.GreenBar[spellCheck.Green.spellID] then
						spellCheck.Green:SetChecked(true)
					else
						spellCheck.Green:SetChecked(false)
					end
					spellCheck.Green:SetScript("OnClick", function()
						GameTooltip:Hide()
						if spellCheck.Green:GetChecked() then
							_G.EasyCCDB.GreenBar[spellCheck.Green.spellID] = true
						else
							_G.EasyCCDB.GreenBar[spellCheck.Green.spellID] = nil
						end
